...
import { Button, Card, Form, Select, DatePicker, Table, Modal, message } from 'antd';
import { PlusOutlined, EyeOutlined } from '@ant-design/icons';
import MainLayout from '../components/Layout/MainLayout';

import GiftApplicationForm from '../components/GiftApplication/GiftApplicationForm';
import '../styles/GiftApplicationPage.css';
...
const GiftApplicationPage: React.FC = () => {
  ...
  const fetchApplications = async (_params?: any) => {
    ...
    try {
      ...
      // 模拟数据
      const mockData: GiftApplication[] = Array.from({ length: 15 }, (_, index) => ({
        id: index + 1,
        ...
        approveName: index % 3 === 0 ? undefined : `审批人${(index % 3) + 1}`,
        approveComment: index % 3 === 0 ? undefined : `审批意见${index + 1}`,
        approveTime: index % 3 === 0 ? undefined : `2026-01-${String(index + 1).padStart(2, '0')}T10:00:00`,
  ...
  const handleReset = () => {
    form.resetFields();
    fetchApplications({ page, size: pageSize });
  };
  ...
  const handleFormSubmit = async () => {
    try {
      // 新增操作
      // await apiClient.post('/gifts/applications', values);
      message.success('礼品申请新增成功');
  ...
  // 处理查看详情
  const handleViewDetail = (_record: GiftApplication) => {
  ...
  return (
    <MainLayout>
      <div className="gift-application-page">
        <Card title="礼品申请" className="application-card">
          {/* 操作按钮区 */}
...
export default GiftApplicationPage;
