...
6. If the corresponding requirement document already exists, please check and ensure that no duplicate generation occurs.
7. **When you need to ask the user a question, ALWAYS use the `ask_user_question` tool with the `options` parameter** to provide preset choices. This lets users click to answer instead of typing. Only omit `options` when the question is truly open-ended with no reasonable preset answers.
